...
var strings = {
  ...
  // opens a floating terminal presenting `omarchy pkg add python` (sudo
  // prompt included) and polls the result. Same pattern as hyprmoncfg.
  ...
  pythonMissingHint: "The backend is a Python script, and python3 is not on this system. "
    ...
    + "showing exactly what will run (omarchy pkg add python) for you to authorize.",
  ...
  pythonNoRuntimeDir: "Could not find the user runtime directory.",
...
// marker files in $XDG_RUNTIME_DIR: ".complete" is touched on success, the
// exit status (a number, "130" = Ctrl-C) is written into ".failed". The
...
// The exact shell line the floating terminal presents and runs. POSIX sh
// only ([ ... ] not (( ... ))) — the presentation terminal is whatever the
...
function pythonInstallCommand(runtimeDir) {
  var marker = pythonMarkerPaths(runtimeDir)
  return "rm -f \"" + marker.failure + "\" \"" + marker.complete + "\""
    + "; status=0; omarchy pkg add python || status=$?"
    + "; if [ \"$status\" -eq 0 ]; then : > \"" + marker.complete + "\""
    + "; else printf '%s\\n' \"$status\" > \"" + marker.failure + "\"; fi"
    + "; (exit \"$status\")"
}

// argv for Process.startDetached: omarchy opens its floating terminal with
// the presentation wrapper around the command — visible, cancellable, and
// gated by sudo. Nothing here runs without the user having pressed the
// panel's Install button.
function pythonInstallArgv(runtimeDir) {
  return ["omarchy", "launch", "floating", "terminal", "with",
          "presentation", pythonInstallCommand(runtimeDir)]
}
